...
void boot_load(lv_obj_t *parent) {
  ...
  if (cameraUtils.checkCameraConnection()) {
  ...
  rlottie_boot =
      lv_rlottie_create_from_raw(parent, 260, 120, lottie_boot_to_stage1_json);
  next_anim_json = lottie_stage1_json;
  lv_rlottie_set_play_mode(rlottie_boot,
                           (lv_rlottie_ctrl_t)(LV_RLOTTIE_CTRL_PAUSE));
      ...
      [](lv_event_t *event) {
        ...
        if (next_anim_json != NULL) {
          if (rlottie_boot_obj->animation) {
            ...
            rlottie_boot_obj->animation =
                lottie_animation_from_data(next_anim_json, next_anim_json, "");
            rlottie_boot_obj->current_frame = 0;
          }
          if (next_anim_json == lottie_stage1_to_2_json) {
            next_anim_json = lottie_stage2_json;
            rlottie_boot_obj->play_ctrl =
                (lv_rlottie_ctrl_t)(LV_RLOTTIE_CTRL_PLAY);
  ...
  lv_timer_create(
      [](lv_timer_t *t) {
        ...
        case STAGE_WAIT_HTTP:
          if (current_bar_pos ==
              STAGE_LOAD_BAR_TOTAL - STAGE_LOAD_HTTP_PCT_STEP * 2) {
