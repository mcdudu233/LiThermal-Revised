...
void MyChart::init(lv_obj_t *parent, uint16_t width, uint16_t height,
                   uint16_t data_interval, uint16_t default_value,
                   MyChartGetValueCallback callback, uint16_t update_interval) {
  ...
  _series = lv_chart_add_series(_chart_widget, lv_palette_main(LV_PALETTE_BLUE),
                                LV_CHART_AXIS_PRIMARY_Y);
  _series2 = lv_chart_add_series(_chart_widget, lv_palette_main(LV_PALETTE_RED),
      ...
      [](lv_timer_t *timer) {
        ...
        if (chart_ptr->_data_weight_current >= chart_ptr->_data_weight_max) {
          ...
          if (chart_ptr->_callback != NULL) {
            ...
            chart_ptr->_data_buffer[1] =
                chart_ptr->_callback(GRAPH_DATA_SOURCE_MIN);
            chart_ptr->_data_buffer2[0] = chart_ptr->_data_buffer2[1];
            chart_ptr->_data_buffer2[1] =
                chart_ptr->_callback(GRAPH_DATA_SOURCE_MAX);
          }
        }
        lv_obj_move_foreground(lv_obj_get_parent(chart_ptr->_chart_widget));
        if (chart_ptr->_data_interval == chart_ptr->_update_interval) {
          lv_chart_set_next_value(chart_ptr->_chart_widget, chart_ptr->_series,
                                  chart_ptr->_data_buffer[1]);
          lv_chart_set_next_value(chart_ptr->_chart_widget, chart_ptr->_series2,
                                  chart_ptr->_data_buffer2[1]);
        } else {
          float next = chart_ptr->cosineInterpolation(chart_ptr->_data_buffer);
          float next2 =
              chart_ptr->cosineInterpolation(chart_ptr->_data_buffer2);
          lv_chart_set_next_value(chart_ptr->_chart_widget, chart_ptr->_series,
                                  next);
          lv_chart_set_next_value(chart_ptr->_chart_widget, chart_ptr->_series2,
                                  next2);
        ...
        lv_coord_t *ser =
            lv_chart_get_y_array(chart_ptr->_chart_widget, chart_ptr->_series);
        lv_coord_t *ser2 =
            lv_chart_get_y_array(chart_ptr->_chart_widget, chart_ptr->_series2);
        ...
        lv_chart_set_range(chart_ptr->_chart_widget, LV_CHART_AXIS_PRIMARY_Y,
                           min_num - 50, max_num + 50);
        lv_chart_set_range(chart_ptr->_chart_widget, LV_CHART_AXIS_SECONDARY_Y,
                           min_num - 50, max_num + 50);
        ...
        //     float pos = (max_num - next) * chart_ptr->_height / (max_num -
        //     min_num); if(pos < 2)
        //         pos = 2;
        //     if(pos + lv_obj_get_height(chart_ptr->label) >=
        //     chart_ptr->_height - 5)
...
void MyChart::showLabel(bool show) {
  if (show) {
    if (label)
      goto end;
...
void MyChart::showTitle(const char *title) {
  if (title) {
    if (lbl_title) {
    ...
    lv_label_set_text(lbl_title, title);
    lv_obj_set_style_bg_color(lbl_title, lv_palette_main(LV_PALETTE_BLUE_GREY),
                              0);
...
uint16_t MyChart::cosineInterpolation(uint16_t data_buffer_ptr[2]) {
  float result = 0.0;
...
static void hideGraph(int delay = 0) {
  lv_anim_move(card.obj, graph_hide_pos[globalSettings.graphPos][0],
               graph_hide_pos[globalSettings.graphPos][1],
               MY_MOVE_ANIM_DEFAULT_TIME, delay);
  lv_obj_fade_out(card.obj, 500, delay);
  is_showed = false;
}

static void showGraph() {
  card.move(graph_hide_pos[globalSettings.graphPos][0],
            graph_hide_pos[globalSettings.graphPos][1], false);
...
const int graph_data_interval_table[7] = {
    40, 100, 500, 1000, 2000, 5000, 10000,
};
...
void graph_show() {
  if (!is_init) {
    ...
    chart.init(card.obj, CARD_SIZE_LARGE_WIDTH, CARD_SIZE_HEIGHT,
               graph_data_interval_table[globalSettings.graphRefreshInterval],
               0, getGraphData,
               graph_data_interval_table[globalSettings.graphRefreshInterval]);
    lv_obj_set_size(chart._chart_widget, CARD_SIZE_SMALL_WIDTH,
                    CARD_SIZE_HEIGHT);
    card.move(graph_hide_pos[globalSettings.graphPos][0],
              graph_hide_pos[globalSettings.graphPos][1], false);
