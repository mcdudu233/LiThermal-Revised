...
static lv_img_dsc_t raw_image_dsc[5];
static lv_color_t raw_image_buffer[5][160 * 120];
...
static photo_type_t getPhotoType(int id) {
  ...
  if (stat(filename_buffer, &s) == 0)
    return PHOTO_TYPE_VIDEO_MJPEG;

  sprintf(filename_buffer, GALLERY_PATH "/CAP%05d.mp4", id);
  if (stat(filename_buffer, &s) == 0)
    return PHOTO_TYPE_VIDEO_MP4;

  sprintf(filename_buffer, GALLERY_PATH "/CAP%05d.jpeg", id);
  if (stat(filename_buffer, &s) == 0)
    return PHOTO_TYPE_PICTURE_JPEG;

  sprintf(filename_buffer, GALLERY_PATH "/CAP%05d.raw", id);
  if (stat(filename_buffer, &s) == 0)
    return PHOTO_TYPE_PICTURE_RAW;

  return PHOTO_TYPE_FILE_NOT_FOUND;
}

// RAW温度数据转灰度图
static void raw_to_grayscale(const std::string &raw_data, lv_color_t *buffer) {
  const int raw_width = 160;
  const int raw_height = 120;

  const float *temperature_data =
      reinterpret_cast<const float *>(raw_data.c_str());

  // 找到温度范围用于归一化
  float min_temp = temperature_data[0];
  float max_temp = temperature_data[0];
  for (int i = 1; i < raw_width * raw_height; ++i) {
  ...
  for (int y = 0; y < raw_height; ++y) {
    for (int x = 0; x < raw_width; ++x) {
      int index = y * raw_width + x;
      float temp = temperature_data[index];
      float normalized = (temp - min_temp) / temp_range;
      uint8_t gray_value = static_cast<uint8_t>(normalized * 255);
      buffer[index] = lv_color_make(gray_value, gray_value, gray_value);
    }
  }
}
...
static void image_obj_render(int obj_id) {
  ...
  case PHOTO_TYPE_PICTURE_RAW: {
    // 读取RAW文件
    char filename_buffer[128];
    sprintf(filename_buffer, GALLERY_PATH "/CAP%05d.raw", image_src_id[obj_id]);
    FILE *file = fopen(filename_buffer, "rb");
    if (file) {
      fseek(file, 0, SEEK_END);
      long file_size = ftell(file);
      fseek(file, 0, SEEK_SET);
      std::string raw_data(file_size, '\0');
      size_t bytes_read = fread(&raw_data[0], 1, file_size, file);
      fclose(file);
      if (bytes_read == file_size) {
        // 转换为灰度图并渲染
        raw_image_dsc[obj_id].header.w = 160;
        raw_image_dsc[obj_id].header.h = 120;
        raw_image_dsc[obj_id].header.cf = LV_IMG_CF_TRUE_COLOR;
        raw_image_dsc[obj_id].data = (uint8_t *)raw_image_buffer[obj_id];
        raw_to_grayscale(raw_data, raw_image_buffer[obj_id]);
  ...
  case PHOTO_TYPE_VIDEO_MP4:
  case PHOTO_TYPE_VIDEO_MJPEG: {
...
static void image_obj_create() {
  LOCKLV();
  lv_draw_img_dsc_init(&canvas_draw_dsc);
  ...
  for (int i = 0; i < 4; ++i) {
    ...
    lv_obj_align(image_obj[i], LV_ALIGN_LEFT_MID, image_pos_x_lut[i],
                 image_pos_y_lut[i]);
    ...
    lv_canvas_set_buffer(canvas_img, canvas_buffer[i], 240, 180,
                         LV_IMG_CF_TRUE_COLOR);
    lv_obj_center(canvas_img);
    if (image_spare_left <= i)
      lv_anim_fly_up(image_obj[i], MY_FLY_ANIM_DEFAULT_HEIGHT,
                     MY_FLY_UP_ANIM_DEFAULT_TIME, image_fly_delay_lut[i]);
    else {
      image_src_id[i] = -1;
    }
  }
  ...
  lv_canvas_set_buffer(canvas_img, canvas_buffer[4], 240, 180,
                       LV_IMG_CF_TRUE_COLOR);
  lv_obj_center(canvas_img);
  for (int i = 0; i < 5; ++i) {
    image_obj_render(i);
...
static void image_obj_slide_left() {
  ...
  lv_anim_move(image_obj[spare_obj], image_pos_x_lut[3], 0,
               MY_MOVE_ANIM_DEFAULT_TIME, 0);
  image_src_id[spare_obj] = getNextImage(
  ...
  for (int i = 1; i < 4 - image_spare_left; ++i) {
    lv_anim_move(image_obj[(image_pointer_left + i) % 5],
                 image_pos_x_lut[i - 1 + image_spare_left],
                 image_pos_y_lut[i - 1 + image_spare_left],
                 MY_MOVE_ANIM_DEFAULT_TIME, image_fly_delay_lut[i]);
  }
...
static void image_obj_slide_right() {
  ...
  lv_obj_set_style_transform_angle(image_obj[spare_obj], 0, 0);
  lv_obj_set_pos(image_obj[spare_obj], image_pos_x_lut[image_spare_left],
                 image_pos_y_lut[image_spare_left]);
  ...
  image_obj_render(spare_obj);
  if (image_src_id[(image_pointer_left + (2 - image_spare_left) + 5) % 5] >
      image_src_id[(image_pointer_left + (3 - image_spare_left) + 5) % 5])
  ...
  for (int i = 0; i < 3 - image_spare_left; ++i) {
    lv_anim_move(image_obj[(image_pointer_left + i) % 5],
                 image_pos_x_lut[i + 1 + image_spare_left],
                 image_pos_y_lut[i + 1 + image_spare_left],
                 MY_MOVE_ANIM_DEFAULT_TIME,
                 image_fly_delay_lut[i + image_spare_left]);
  }
  lv_anim_move(image_obj[(image_pointer_left + 3 - image_spare_left) % 5],
...
static void image_obj_del_current() {
  ...
  lv_my_anim_fall_down(
      image_obj[(image_pointer_left + 3 - image_spare_left) % 5],
      MY_POP_FALL_ANIM_DEFAULT_TIME, 0, false);
  if (totalImages < 4) {
...
static void image_obj_close() {
  for (int i = 0; i < 4 - image_spare_left; ++i) {
    lv_anim_fall_fly(image_obj[(image_pointer_left + i) % 5],
                     MY_FLY_ANIM_DEFAULT_HEIGHT, MY_FLY_DOWN_ANIM_DEFAULT_TIME,
                     image_fly_delay_lut[i + image_spare_left]);
    lv_obj_del_delayed(image_obj[(image_pointer_left + i) % 5],
                       MY_FLY_DOWN_ANIM_DEFAULT_TIME +
                           image_fly_delay_lut[i + image_spare_left]);
  }
...
static void full_screen_show(int id) {
  ...
  case PHOTO_TYPE_PICTURE_JPEG: {
    ...
    lv_canvas_set_buffer(ffmpeg_fullscreen, canvas_fullscreen_buffer, 320, 240,
                         LV_IMG_CF_TRUE_COLOR);
    canvas_draw_dsc.zoom = 256;
    lv_canvas_draw_img(ffmpeg_fullscreen, 0, 0, file_name_buffer,
                       &canvas_draw_dsc);
  ...
  case PHOTO_TYPE_PICTURE_RAW: {
    // 读取RAW文件
    sprintf(file_name_buffer, GALLERY_PATH "/CAP%05d.raw", id);
    ...
    if (file) {
      ...
      if (bytes_read == file_size) {
        // 转换为灰度图并渲染
        raw_to_grayscale(raw_data, ffmpeg_fullscreen_raw_img_buffer);
        ffmpeg_fullscreen_raw_img_dsc.header.w = 160;
        ffmpeg_fullscreen_raw_img_dsc.header.h = 120;
        ffmpeg_fullscreen_raw_img_dsc.header.cf = LV_IMG_CF_TRUE_COLOR;
        ffmpeg_fullscreen_raw_img_dsc.data =
            (const uint8_t *)ffmpeg_fullscreen_raw_img_buffer;

        ffmpeg_fullscreen = lv_canvas_create(lv_layer_top());
        lv_canvas_set_buffer(ffmpeg_fullscreen, canvas_fullscreen_buffer, 320,
                             240, LV_IMG_CF_TRUE_COLOR);
        canvas_draw_dsc.zoom = 514;
        lv_canvas_draw_img(ffmpeg_fullscreen, 0, 0,
                           &ffmpeg_fullscreen_raw_img_dsc, &canvas_draw_dsc);
        lv_obj_fade_in(ffmpeg_fullscreen, 500, 0);
      } else {
        printf("Failed to read RAW file: %s (read %zu, expected %ld)\n",
               file_name_buffer, bytes_read, file_size);
      }
    } else {
      printf("Cannot open RAW file: %s\n", file_name_buffer);
    }
...
void menu_gallery_show() {
  ...
  image_obj_create();
  LOCKLV();
  card_gallery.create(lv_layer_top(), 0, GALLERY_CARD_HIDE_Y, 120, 33,
                      LV_ALIGN_TOP_MID);
  ...
  lv_obj_set_style_bg_opa(card_gallery.obj, 128,
