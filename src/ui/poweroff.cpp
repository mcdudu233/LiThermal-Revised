...
static bool poweroff_started = true;
...
void poweroff_show() {
  ...
  if (lbl_power_off_prompt == NULL || !lv_obj_is_valid(lbl_power_off_prompt)) {
  ...
  lv_timer_create(
      [](lv_timer_t *timer) {
        ...
        lv_label_set_text_fmt(lbl_power_off_prompt, "长按 %d 秒后关机",
                              ((255 - animation_opa) >> 6) + 1);
        if (poweroff_started) {
          if (animation_opa == 255) {
            ...
            printf("Poweroff!\n");
            settings_save();
            global_poweroff_request =
                true; // 此处通知HAL线程关机，然后返回，在此注册背光渐暗动画。主线程等待动画完全结束（500ms左右）再启动关机流程
            lv_timer_del(timer);
            return;
          } else {
            animation_opa += 2;
