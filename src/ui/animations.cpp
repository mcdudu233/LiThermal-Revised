...
float my_ease_out(float t) { return 1 - (1 - t) * (1 - t) * (1 - t); }
...
int32_t my_anim_path_ease_in_out(const lv_anim_t *a) {
  ...
  float new_value;
  new_value =
      easeInOutQuart(t) * (a->end_value - a->start_value) + a->start_value;
  return new_value;
}
...
void lv_my_anim_fall_down(lv_obj_t *obj, uint16_t time, uint16_t delay,
                          bool del) {
  static lv_anim_t a;
  lv_coord_t y_start;
  lv_anim_del(obj, NULL);
  ...
  if (del) {
    lv_anim_set_ready_cb(
        &a, [](lv_anim_t *a) { lv_obj_del((lv_obj_t *)(a->var)); });
  } else {
    lv_anim_set_ready_cb(&a, [](lv_anim_t *a) {
...
void lv_anim_move(lv_obj_t *obj, lv_coord_t x, lv_coord_t y, uint16_t time,
                  uint16_t delay) {
  lv_anim_t a;
  int16_t p;
  lv_anim_init(&a);
...
void lv_anim_size(lv_obj_t *obj, lv_coord_t w, lv_coord_t h, uint16_t time,
                  uint16_t delay) {
  lv_anim_t a;
  int16_t p;
  lv_anim_init(&a);
...
void lv_anim_fly_up(lv_obj_t *obj, lv_coord_t h, uint16_t time,
                    uint16_t delay) {
  lv_anim_t a;
  int16_t p;
  lv_anim_del(obj, NULL);
...
void lv_anim_fall_fly(lv_obj_t *obj, lv_coord_t h, uint16_t time,
                      uint16_t delay) {
  lv_anim_t a;
  int16_t p;
  lv_anim_del(obj, NULL);
