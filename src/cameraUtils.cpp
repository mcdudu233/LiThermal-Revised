...
static std::vector<std::string> split_multipart(const std::string &body,
  ...
  while (start != std::string::npos) {
    ...
    std::string this_part_length_str =
        body.substr(start_length + 15, end_length);
...
void CameraUtils::getTemperature() {
  ...
  if (res && res->status == 200) {
    ...
    cJSON *ThermometryRulesTemperatureInfoList =
        cJSON_GetObjectItem(root, "ThermometryRulesTemperatureInfoList");
    cJSON *ThermometryRulesTemperatureInfo = cJSON_GetObjectItem(
        ThermometryRulesTemperatureInfoList, "ThermometryRulesTemperatureInfo");
    ...
    cJSON *MaxTemperaturePoint =
        cJSON_GetObjectItem(pack, "MaxTemperaturePoint");
    cJSON *MinTemperaturePoint =
        cJSON_GetObjectItem(pack, "MinTemperaturePoint");
    ...
    lastResult.isFreezedata =
        cJSON_GetObjectItem(pack, "isFreezedata")->valueint;
    cJSON_Delete(root);
  } else {
...
std::string CameraUtils::getTemperatureAll() {
  auto res = cli.Get("/ISAPI/Thermal/channels/1/thermometry/"
...
float CameraUtils::getTemperatureCenter() {
  // 取中心位置
  lastResult.centerTemperature = *(
      float *)((uint8_t *)getTemperatureAll().c_str() + ((80 + 60 * 160) * 4));
  return lastResult.centerTemperature;
}

const char *color_palette[] = {
    "WhiteHot", "BlackHot", "Fusion1", "Fusion2",  "Rainbow",
    "Ironbow1", "Ironbow2", "Sepia",   "Color1",   "Color2",
    "IceFire",  "Rain",     "RedHot",  "GreenHot", "DeepBlue",
};

void CameraUtils::setColorPalette(int palette) {
  std::string data =
      "<?xml version=\"1.0\" encoding=\"UTF-8\"?><Palettes><mode>";
...
void CameraUtils::saveJpegWithExtra(const char *save_filename, bool saveJpeg,
                                    bool saveExtra) {
  auto res = cli.Get("/ISAPI/Thermal/channels/1/thermometry/"
                     "jpegPicWithAppendData?format=json");
  if (res && res->status == 200) {
    ...
    if (save_filename != NULL) {
      if (saveJpeg) {
        std::ofstream fileJpeg(std::string(save_filename) + ".jpeg");
        if (fileJpeg.is_open()) {
          fileJpeg << parts[1];
          fileJpeg.close();
        }
      ...
      if (saveExtra) {
        std::ofstream fileRaw(std::string(save_filename) + ".raw");
        if (fileRaw.is_open()) {
          fileRaw << parts[2];
          fileRaw.close();
        }
...
void CameraUtils::setCenterMeasure(bool en) {
  auto res =
      cli.Get("/ISAPI/Thermal/channels/1/temperatureCorrect?format=json");
  char data[1024];
  if (res && res->status == 200) {
    cJSON *root = cJSON_Parse(res->body.c_str());
    cJSON *TemperatureCorrect = cJSON_GetObjectItem(root, "TemperatureCorrect");
    cJSON *streamOverlay =
        cJSON_GetObjectItem(TemperatureCorrect, "streamOverlay");
    cJSON_SetBoolValue(streamOverlay, en);
    cJSON_PrintPreallocated(root, data, 1024, cJSON_False);
    cJSON_Delete(root);
    cli.Put("/ISAPI/Thermal/channels/1/temperatureCorrect?format=json", data,
            "application/json");
  }
}

void CameraUtils::calibrateManually() {
  cli.Put("/ISAPI/Image/channels/1/ManualShutterCorrect", "",
          "application/xml");
}
...
void CameraUtils::set4117Cursor(bool min, bool max) {
  ...
  if (res && res->status == 200) {
    ...
    while (1) {
      ...
      if (line.find("displayMax") != std::string::npos) {
        if (max) {
          ...
          result += "<displayMaxTemperatureEnabled>false</"
                    "displayMaxTemperatureEnabled>\n";
        }
      } else if (line.find("displayMin") != std::string::npos) {
        if (min) {
          result += "<displayMinTemperatureEnabled>true</"
    ...
    cli.Put("/ISAPI/Thermal/channels/1/thermometry/basicParam", result,
