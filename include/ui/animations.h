...
#include "build/toolchain-sunxi-musl/toolchain/include/math.h"
#include "build/toolchain-sunxi-musl/toolchain/include/stdint.h"
#include "build/toolchain-sunxi-musl/toolchain/include/stdio.h"
#include "build/toolchain-sunxi-musl/toolchain/include/stdlib.h"
#include "build/toolchain-sunxi-musl/toolchain/include/string.h"
#include "lvgl.h"
...
void lv_my_anim_pop_up(lv_obj_t *obj,
                       uint16_t time = MY_POP_FALL_ANIM_DEFAULT_TIME,
                       uint16_t delay = 0);
// 使用lv_my_anim_fall_down函数添加的对象，禁止修改其userdata
void lv_my_anim_fall_down(lv_obj_t *obj,
                          uint16_t time = MY_POP_FALL_ANIM_DEFAULT_TIME,
                          uint16_t delay = 0, bool del = true);
void lv_anim_move(lv_obj_t *obj, lv_coord_t x, lv_coord_t y,
                  uint16_t time = MY_MOVE_ANIM_DEFAULT_TIME,
                  uint16_t delay = 0);
void lv_anim_size(lv_obj_t *obj, lv_coord_t w, lv_coord_t h,
                  uint16_t time = MY_SIZE_ANIM_DEFAULT_TIME,
                  uint16_t delay = 0);
void lv_anim_fly_up(lv_obj_t *obj, lv_coord_t h = MY_FLY_ANIM_DEFAULT_HEIGHT,
                    uint16_t time = MY_FLY_UP_ANIM_DEFAULT_TIME,
                    uint16_t delay = 0);
void lv_anim_fall_fly(lv_obj_t *obj, lv_coord_t h = MY_FLY_ANIM_DEFAULT_HEIGHT,
                      uint16_t time = MY_FLY_DOWN_ANIM_DEFAULT_TIME,
                      uint16_t delay = 0);
...
public:
  lv_obj_t *obj = NULL;
  MyCard(lv_obj_t *parent, lv_coord_t x = 0, lv_coord_t y = 0, lv_coord_t w = 0,
         lv_coord_t h = 0, lv_align_t align = LV_ALIGN_TOP_LEFT) {
    if (parent != NULL) {
      create(parent, x, y, w, h, align);
    }
  }
  MyCard() {}
  void create(lv_obj_t *parent, lv_coord_t x = 0, lv_coord_t y = 0,
              lv_coord_t w = 0, lv_coord_t h = 0,
